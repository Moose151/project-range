...
- **Spectrum Plan on signal package page:** A collapsible **Spectrum Plan** section appears on the package edit page whenever the package has signals. It renders a canvas chart showing each signal as an occupied-bandwidth block — centred on TX/RX IF by default, with an IF/RF selector when operators need the RF plan. Width is determined by `(symbol rate ÷ modulation rate) × (1 + rolloff 0.25) / 1000 MHz`, and height by power (dBm). Controls: **Centre (MHz)** to pan, **Span (MHz)** to zoom, **Guard Left / Guard Right** frequency markers, **TX + RX / TX only / RX only**, and **IF / RF**. Guard band boundaries draw amber dashed lines and shade areas outside the permitted range in red. All settings are saved per-package in the browser.
...
- **Live Spectrum dashboard widget:** A new optional dashboard widget (**Widgets → Live Spectrum**) draws a live IF spectrum of transmitted signals from active serials, with an RF toggle available. It now refreshes from the active serial signal-table update flow and immediately after dashboard bulk-submit changes, rather than using a separate 30-second timer. Only signals currently **Up** are shown. The same centre freq, span, and guard band controls are available per widget instance.
...
- **Cross-workspace copy fix:** Copy to Sandbox / Copy to Live now preserves the explicit target workspace instead of being overwritten by the current range-state scoping hook. This fixes packages, serials, and CDA tables copying back into the workspace they were already in.
- **Sandbox hardware-sync pause:** Admin Config → System now has a **Pause hardware sync in Sandbox** toggle, and Testing mode shows a quick **Sandbox Sync** toggle at the bottom of the sidebar. When enabled and the range state is Testing, automatic and force EBEM/CBM sync plus active SNMP polling are skipped so operators can rehearse manual signal/routing changes without modem or matrix reads overwriting them. Live/standard operation is unaffected.
