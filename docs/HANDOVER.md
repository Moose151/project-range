...
**App name:** "SEW Range" (re-branded from "Project Range"). **Version:** `0.31.7` (single source: `app/config.py` `APP_VERSION`, shown in the top-right of the UI near the theme toggle).
...
**prioritised, low-risk-first** plan. Step 1 is **done** (0.31.7); the rest are
ordered by value-for-effort. Measure before/after with the browser Network tab
(request count + timing) and, for DB work, SQLite `EXPLAIN QUERY PLAN`.
...
2. **Enable GZip + long-lived static caching.** Add `app.add_middleware(GZipMiddleware, minimum_size=500)` and serve `/static` with `Cache-Control: public, max-age=31536000, immutable` (safe because every asset URL carries `?v=NN`; bump the version to bust). Cheap, no behaviour change.
3. **Collapse the poll storm into one heartbeat.** Replace the many independent timers with a single `GET /status/heartbeat` (say every 5–10s) returning one JSON payload (range state, buzzer, up-count, serial list summary, CEASE state, chat-unread) and fan it out to the widgets client-side. Dial `pollKnownChatRooms` (2.5s) and CEASE (3s) back to ≥5s. Cuts request volume by ~5–8×. Pause polling when `document.hidden` (backgrounded tab).
4. **Kill the N+1s in `_dashboard_ctx`.** Batch-load per serial: fetch the latest log per signal in **one** window/group query instead of scan-and-dedupe-in-Python (`_latest_signal_status`); resolve all package RF configs and priorities for the serial in single queries keyed by signal name; `selectinload`/`joinedload` `package_links → package → signals`. Target: constant queries per poll regardless of signal count.
...
7. **Consider WAL + connection pooling tuning for SQLite** (`PRAGMA journal_mode=WAL`, `synchronous=NORMAL`) so concurrent polls + writes don't block each other.

**Do NOT** reach for a bigger server or a rewrite first — items 1–4 are the 80/20 and are all local, reversible changes. Re-measure after each.
