...
**Root causes (why it feels slow):**
1. **Un-indexed hot table.** `signal_logs` grows forever and the dashboard polled it every 5s with no indexes → full scan per poll. **← fixed in 0.31.7.**
2. **Poll storm.** Every open tab fires many independent timers: dashboard fragment **5s per serial widget**, `/status/buzzer` 10s, `/status/active-count` 10s, `/status/serials` 15s, CEASE 3s, chat state 10s, **known-chat-rooms 2.5s**, plus a range-state poll 5s. Each is a full request through middleware + fresh DB queries. On a multi-serial dashboard that's a lot of concurrent work every few seconds.
3. **N+1 queries per poll.** `_dashboard_ctx` loops active serials and, per signal, calls `serial_package_rf_config` (`_pkg_rf_by_signal`) and resolves priority/CBM status — each issuing its own query. Also lazy-loaded relationships (`serial.package_links` → `package.signals`) fire per-row SELECTs.
4. **No response compression / weak static caching.** No GZip middleware, so HTML/JS/CSS go over the wire uncompressed. `StaticFiles` is mounted without a long `Cache-Control` even though assets are already cache-busted with `?v=NN`, so the browser revalidates each one.
5. **Unbounded tables.** `signal_logs` and `audit_logs` are never pruned/archived; every scan and export gets heavier over time.
...
- **0.31.9 — Performance pass 3 (poll-storm → single heartbeat):**
  ...
  - **Kick/idle auto-redirect preserved on every page:** the heartbeat detects the `/login` redirect (`response.redirected`) and navigates there. Previously only the per-badge HTMX pollers (removed here) did this; the dashboard fragment poll still triggers the 0.31.7 `HX-Redirect` path too.
  - The old `/status/*`, `/range-state/status`, `/cease/state` endpoints are kept for direct callers/back-compat but are no longer polled on a timer. `base.html` badges + the dashboard active-signals widget lost their `hx-get`/`hx-trigger` (heartbeat fills them). Verified end-to-end (login → heartbeat JSON → dashboard 200).
- **0.31.8 — Performance pass 2 (compression, WAL, dashboard N+1):**
  - **GZip compression** on all responses (`GZipMiddleware(minimum_size=500)`, added before `SessionMiddleware` so Session stays outermost). HTML/JS/CSS/JSON now go over the LAN compressed — verified `Content-Encoding: gzip` + `Vary: Accept-Encoding`.
  - **Static caching:** `/static` served via a `CachedStaticFiles` subclass adding `Cache-Control: public, max-age=604800` (1 week). Safe: JS/CSS are `?v=NN`-busted; other assets change rarely.
  - **SQLite WAL** (`app/database.py` connect listener: `journal_mode=WAL`, `synchronous=NORMAL`, `busy_timeout=5000`) so poll reads and background sync writes stop blocking each other. `scripts/backup_db.py` now checkpoints (`wal_checkpoint(TRUNCATE)`) before copying; `*.db-wal`/`*.db-shm` gitignored.
  - **Dashboard N+1 reductions:** `_latest_signal_status` now selects only the latest row per signal (a `max(id)` group-by subquery over the new index) instead of loading every historical row for the serial and de-duping in Python each 5s poll; `_pkg_rf_by_signal` batch-loads the serial's packages+signals once (`selectinload`) and resolves in Python instead of calling `serial_package_rf_config` per row. Both verified to return byte-identical results to the old logic (incl. ordering, deleted/`[NOTE]` exclusion). Remaining N+1s (CDA loop, `has_cbm_mapping` lazy-load) noted in the performance plan.
- **0.31.7 — Session-kick login fix + first performance pass:**
  - **Fixed the "tiled/jumping login screen" on session kick/idle-out.** When a session was invalidated (idle timeout, or the same account logging in elsewhere → `active_session_token` mismatch), `get_current_user` raises a 302 to `/login`. The dashboard/base-layout **HTMX pollers** (dashboard fragment 5s, `/status/*` 10–15s) transparently *followed* that 302 to `/login` and swapped the **full login page** into each small poll target — tiling duplicate login forms across the screen, which needed a manual refresh. Fix: `app/main.py` `redirect_handler` now detects the `HX-Request` header and returns **`204` + `HX-Redirect`** so HTMX performs a real full-page navigation to `/login`; normal (non-HTMX) requests still get the plain 302. Net effect: a kicked terminal cleanly bounces to the login page on the next poll instead of tiling.
  - **First performance pass — database indexes.** `signal_logs` had **no indexes**, yet the dashboard poll runs a latest-log-per-signal query (plus active-signal/CBM/priority lookups) every 5s — a full table scan each time, which degrades as logs accumulate. Added composite/covering indexes via `SignalLog.__table_args__` (fresh DBs) **and** idempotent `CREATE INDEX IF NOT EXISTS` in `init_db._migrate` (existing DBs): `ix_signal_logs_serial_signal_ts (serial_id, signal_name, timestamp)`, `ix_signal_logs_signal_ts`, `ix_signal_logs_serial_id`, plus indexes on `audit_logs(timestamp)`, `serials(is_started, closed_at)`, `signal_package_entries(package_id)`, `serial_packages(serial_id)`. Verified the poll query now plans `SEARCH … USING INDEX ix_signal_logs_serial_signal_ts` instead of `SCAN`. See **[Performance / efficiency plan](#-performance--efficiency-plan-how-to-de-sluggify)** for the remaining prioritised work.
- **0.31.6 — Dashboard signal-rename history + universal widget close icon:**
  - **Rename history is now preserved in Logs/History (not on the dashboard).** Dashboard quick-edit already renames a signal (`_rename_dashboard_signal` in `dashboard.py`, rewrites the package entries + all its logs to the new name so the dashboard shows one identity). It now also drops a narrative `[NOTE]` row (`entry_type="SerialRename"`, notes `Signal renamed: OLD → NEW`, scoped to the serial). `[NOTE]` rows are filtered out of the dashboard widget (`_latest_signal_status`), so the name trail shows only in `/logs` and serial history. `history_detail.html`'s narrative branch now also matches `SerialRename` / any `[NOTE]` row; `logs_list.html` already keyed off `signal_name == '[NOTE]'`.
  - **Universal widget close icon:** the Clock, CDA, Serial-notes and Signals widget headers used a `bi-eye-slash` (eye) to hide; utility widgets used a `bi-x-lg` (X). All widget close controls are now `bi-x-lg` for consistency (tooltips still read "Hide …" since these widgets are re-showable from the Widgets menu, unlike utility widgets which are removed).
- **0.31.5 — Dashboard source-assignment + effects-dropdown fixes:**
  - **CBM sync button now appears when a modem source is assigned from the dashboard** (previously only after a full page reload). The `has_cbm_mapping` state that gates the header **CBM** button lives in the serial widget header, but the quick-edit/bulk-update/reorder/add responses only swap the signal-table fragment — so a newly created CBM mapping wasn't reflected. Fix: every fragment response now passes `has_cbm_mapping` (new `_serial_has_cbm_mapping(db, serial_id)` in `dashboard.py`); `signal_table.html` renders a hidden `.cbm-map-flag` carrying it; the store always renders the CBM `<form class="serial-cbm-form">` (hidden via `d-none` when unmapped); JS `applyCbmMapping(scope)` toggles the button in the store + visible header after each swap (and every 5s poll). Backend source→`SignalPackageEntry.cbm_device_id` persistence was already correct — this was purely the header not refreshing, which also blocked the manual sync path and made it look like "no modem updates".
  - **Effects dropdown no longer clipped by the widget bottom.** The signal-table body is `overflow-auto` (wide-table scroll), which clipped the absolutely-positioned effects menu on short widgets (1–2 signals). Fix: the effects toggle gained `.effects-dd-toggle` and is initialised with Popper's `strategy: 'fixed'` (`initEffectDropdowns()`), so the menu renders `position: fixed` and escapes the overflow container. Re-run on load + every table swap. (No `app.js`/`app.css` bump — all changes are inline dashboard JS + server templates.)
...
- **0.31.0 — Activity lifecycle, serial editing, spectrum toggles:**
  ...
  - Package spectrum: per-signal **show/hide** toggles (localStorage `spectrum_hidden_pkg_{id}`), All/None; `renderSpectrum` uses `visibleSpecSignals()`.
  - Signal drag-reorder already existed (`/packages/{id}/signals/reorder` + SortableJS).
- **0.30.0–0.30.3 — UI polish:** sticky table headers + signal defaults (Power −35 / Inner TURBO); global **＋ New** top-bar dropdown + package breadcrumb; shared in-app **confirm modal** (`data-confirm`, 37 forms); **dashboard presets** (save/switch named layouts). Also fixed `/config` 500 in Sandbox (0.28.3).
...
- **0.29.1 — Package & clock improvements:**
  ...
  - **Inner code default `TURBO`** on the add-signal form (editable; `resetForm` restores it; edit populates the real value).
  - **Spectrum Plan stays open** across signal add/edit reloads (`spectrum_open_pkg_{id}` in localStorage + restore).
  - **Clocks reworked** (`dashboard.html`): "Add Clock" menu item adds multiple; each clock is compact with its own **timezone `<select>`** (`Intl.supportedValuesOf('timeZone')`, stored as `config.tz`) + a Zulu reference line. `updateUtilityClocks` rewritten; main clock CSS shrunk. `app.css?v=37`.
- **0.29.0 — Dashboard masonry tiling** (half-width widgets pack without dead space; ResizeObserver row-span, `.masonry-ready` fallback).
- **0.28.3 — Fixed `/config` 500 in Sandbox mode** (context var shadowed the `sandbox_hardware_sync_paused` template global) + hardened system-health probes.
- **0.28.2 — Removed redundant BER badge from the EBEM Sync cell** (BER keeps its own column).
- **0.28.1 — PDF uploads for all roles with admin approval:**
  - `DocAttachment.approval_status` (`approved`|`pending`, default `approved`) + migration `ALTER TABLE doc_attachments ADD COLUMN approval_status VARCHAR(16) DEFAULT 'approved'`.
  - `docs.docs_upload_attachment`: admins → `approved`; everyone else → `pending`. `docs_approve_attachment` (admin only). `docs_delete_attachment` now enforces ownership (admin any = reject; owner own) and `docs_get_attachment` refuses to serve a pending PDF to anyone but an admin/uploader.
  - `main._observer_write_allowed` allowlists `POST /docs/*/attachments` (upload) for observers — **not** approve/delete — so observers can submit a PDF for approval but not manage them.
  - `docs_page.html`: header **Upload PDF** shown to all roles; sidebar splits **approved** (everyone) vs **pending** (admins see all, uploaders see own) with Approve/Reject; inline embeds only render approved PDFs; non-admins see an "awaiting approval" note.
...
- **0.26.0 — Effects rename, priority, serial instructions, CDA units, PDF docs, chat memory:**
  ...
  - **Serial instructions:** `Serial.instructions` (Text). Set on create (`serials.py` `serial_create`), copied in `copy-to-other`, editable any time before close via new `POST /serials/{id}/details` (updates notes+instructions). Displayed as an info banner on the dashboard serial widget and on the Serials page (with an inline collapse edit form on active serials, read-only banner on pending).
  - **CDA power units:** `CDAWindow.max_power_unit` (dBm|dBW|W). Canonical value still stored in dBm; entered value converted via `_to_dbm(value, unit)` in `cda.py` on window add/edit; unit preserved for round-trip editing. Model props `max_power_dbw`, `max_power_w`, `max_power_all_label` render all three units in the CDA editor, dashboard CDA widget table, and live countdown banner (windows JSON gains `max_power_dbw`/`max_power_w`/`max_power_all`).
  - **PDF attachments on docs:** new `DocAttachment` model (table `doc_attachments`, files on disk under `DATA_DIR/doc_attachments`, gitignored). Routes in `docs.py`: `POST /docs/{slug}/attachments` (upload, PDF-only, ≤20 MB `MAX_DOC_ATTACHMENT_BYTES`), `GET /docs/{slug}/attachments/{id}` (inline serve via `FileResponse`), `POST /docs/{slug}/attachments/{id}/delete`. `docs_page.html` shows a side-panel list (view/download/remove) plus inline `<embed>` of each PDF below the article.
  - **Dashboard chat remembers open room:** `saveUtilityWidgets()` now persists chat `config.selectedRoom`; `renderUtilityWidget` restores `dataset.selectedRoom` before `renderDashboardChatWidget()`, which now re-enables input + loads messages when the saved room exists; `selectDashboardChatRoom` calls `saveUtilityWidgets()` so the choice is remembered immediately.
  - No `app.css`/`app.js` cache bump needed (all changes are in server-rendered templates + Python; inline dashboard JS is not cached).
