...
- **[shipped 0.31.8]** GZip compression, week-long static caching, SQLite WAL, and the biggest dashboard N+1s removed (`_latest_signal_status` latest-per-signal subquery; `_pkg_rf_by_signal` batched).
- **[shipped 0.31.9]** Poll storm collapsed into a single `/status/heartbeat` (five timers → one), paused when the tab is hidden.
- **[next]** ETag/304 the heartbeat + fragment polls; retention/archive job for `signal_logs`/`audit_logs`; finish the last `_dashboard_ctx` N+1s (CDA loop, `has_cbm_mapping`).
...
## 🎨 UI polish batch (0.30.x — in progress)
...
- **[shipped 0.28.0]** **Encrypt admin-only documents at rest** — Fernet-encrypt `DocPage.content` (and version snapshots) for `visibility == 'admins'` so they aren't plaintext in `range.db`. Verified round-trip. Trade-off: encrypted docs aren't full-text-searchable by body (title/tags still work); SECRET_KEY must be stable (same caveat as modem passwords).
- **[shipped 0.28.0]** **BER estimate** from CBM/EBEM sync — `RX_BEREST` shown on the dashboard (hideable BER column + EBEM cell) and in exports, with an admin threshold-logging setting (off by default). *(Previous-assistant work, adopted & documented.)*
- **[shipped 0.28.0]** **"Documentation" → "Wiki"** UI rename; **effect dropdown** now closes on selection.
- **[shipped 0.28.1]** **PDF upload permissions + approval** — admins upload directly; users & observers may upload but their PDFs need admin approval before appearing.
- **[shipped 0.29.0]** **Dashboard masonry** — half-width widgets tile tightly without dead space.
- **[shipped 0.29.1]** Closed-loop **TxIF↔RxIF mirror**; **package chameleon +** (planned `-N` chameleons, count continues to the dashboard); **inner code defaults to TURBO**; **Spectrum Plan stays open** on signal add/edit; **clocks reworked** (multiple, per-clock timezone, compact).
- **[shipped 0.29.2]** **Activity workflow** — create a pending serial *and* clone a serial (packages/CDA/notes) directly on the activity page. Fixed chameleon numbering to be scoped (fresh signal starts at `-1`; planned chameleons still continue to the dashboard).
...
- **0.26.6:** Effect log entries render as readable event rows (signal, effect badge, source, Eb/No, carrier/channel/mod lock) instead of a normal signal row with details hidden in Notes.
- **0.26.5:** Startup cleanup downs any signal left **Up** on an already-closed serial (idempotent).
...
- [x] **Documentation wiki usability pass** — cleaner search/browse home layout,
  category counts, real Uncategorised filtering, less noisy page rows, and a
  compact page action toolbar.
...
- [x] **Operator readiness surfacing** — package health badges, serial readiness
  badges, and handover Open Issues now surface missing setup and live exceptions
  without opening each underlying page.
